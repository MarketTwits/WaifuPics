...
package com.markettwits.core_ui.components.zoomable
 ...
 *
 * A caller of this function can choose if the pointer events will
 * be consumed. And the caller can implement [onGestureStart] and
 * [onGestureEnd] event.
 *
 * @param canConsumeGesture Lambda that asks the caller whether the gesture
 *     can be consumed.
 * @param onGesture This lambda is called when [canConsumeGesture] returns
 *     true.
 ...
 * @param enableOneFingerZoom If true, enable one finger zoom gesture,
 *     double tap followed by vertical scrolling.
 */
 ...
 *
 * @param onCancel Callback function that will be called if PointerEvents
 *     is consumed by other composable.
 * @param action Callback function that will be called every PointerEvents
 *     occur.
 */
 ...
 * @return If the second down event comes before timeout, returns it. If
 *     not, returns null.
 */
 ...
 * Touch slop detector.
 *
 * This class holds accumulated zoom and pan value to see if touch slop is
 * past.
 *
 * @param threshold Threshold of movement of gesture after touch down. If
 *     the movement exceeds this value, it is judged to be a swipe or zoom
 *     gesture.
 */
...
/**
 * [ScrollGesturePropagation] defines when [Modifier.zoomable] propagates
 * scroll gestures to the parent composable element.
 */
enum class ScrollGesturePropagation {

    /**
     * Propagates the scroll gesture to the parent composable element when the
     * content is scrolled to the edge and attempts to scroll further.
     */
    ContentEdge,

    /**
     * Propagates the scroll gesture to the parent composable element when the
     * content is not zoomed.
     */
    NotZoomed,
}
 ...
 * @param zoomEnabled specifies if zoom behaviour is enabled or disabled.
 *     Even if this is false, [onTap] and [onDoubleTap] will be called.
 * @param enableOneFingerZoom If true, enable one finger zoom gesture,
 *     double tap followed by vertical scrolling.
 * @param scrollGesturePropagation specifies when scroll gestures are
 *     propagated to the parent composable element.
 * @param onTap will be called when single tap is detected on the element.
 * @param onDoubleTap will be called when double tap is detected on the
 *     element. This is a suspend function and called in a coroutine scope.
 *     The default is to toggle the scale between 1.0f and 2.5f with
 *     animation.
 */
 ...
 *
 * @param targetScale Scale to be set if this function is called when the
 *     scale is 1.0f.
